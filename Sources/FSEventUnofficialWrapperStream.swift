...
///
public final class FSEventUnofficialWrapperStream {
    ...
    private let handler: (FSEventUnofficialWrapperStreamEvent) -> ()
     ...
     */
    public init(pathsToWatch: [String], sinceWhen: FSEventUnofficialWrapperStreamEventID, latency: TimeInterval, flags: FSEventUnofficialWrapperStreamCreateFlags, handler: @escaping (FSEventUnofficialWrapperStreamEvent) -> ()) throws {
        ...
        let callback: CoreServices.FSEventStreamCallback = { (
            ...
            guard numEvents == eventPaths2.count else {
                FSEventUnofficialWrapperStreamIllogicalErrorLog(code: .unmatchedEventParameterCounts, message: "Event count is `\(numEvents)`, but path count is `\(eventPaths2.count)`").cast()
                return
            }
            let unmanagedPtr: Unmanaged<FSEventUnofficialWrapperStream> = Unmanaged.fromOpaque(clientCallBackInfo1)
        ...
        // ultimately trigger copying inside og the system framework...
        ...
        guard let newRawref = FSEventStreamCreate(nil, callback, context1, pathsToWatch1, sinceWhen1, latency1, flags1) else {
            throw FSEventUnofficialWrapperStreamError(code: .cannotCreateStream)
        }
        rawref = newRawref
    }
    deinit {
 ...
 *  Accessors
 */
extension FSEventUnofficialWrapperStream {
    /*
    ...
    public func getLatestEventID() -> FSEventUnofficialWrapperStreamEventID {
        let eventId = FSEventStreamGetLatestEventId(rawref)
        let eventID1 = FSEventUnofficialWrapperStreamEventID(rawValue: eventId)
        return eventID1
    }
...
extension FSEventUnofficialWrapperStream {
    ...
    public func start() throws {
        ...
        case false:
            throw FSEventUnofficialWrapperStreamError.init(code: .cannotStartStream)
        case true:
            return
        }
    }
     ...
     */
    @available(OSX 10.5, *)
    public func flushAsync() -> FSEventUnofficialWrapperStreamEventID {
...
extension FSEventUnofficialWrapperStream: CustomStringConvertible, CustomDebugStringConvertible {
